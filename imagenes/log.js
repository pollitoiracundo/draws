...
$(document).ready(function() {
    ...
    var root = "https://chat-f569a.firebaseio.com/";
    var fb = new Firebase(root);
    
    // Listen for new sessions
    fb.limit(1000).on("child_added", function(snapshot) {
        var sessionid = snapshot.key();
        var client_name = snapshot.val().client_name;
        var date = snapshot.val().date;
        globali++;
        $("tbody#sessions")
            .prepend('<tr><td>'+globali+'</td><td>' + date + '</td><td>' + client_name + '</td><td><span class="sessionid badge">' + sessionid + '</span></td></tr>');
